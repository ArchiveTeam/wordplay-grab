wordplay-grab
=============
This is a script you can run to grab the Wordplay site.


More information about the archiving project can be found on the ArchiveTeam wiki: [Wordplay](https://wiki.archiveteam.org/index.php/Wordplay)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "Wordplay" project in the Warrior interface.
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/wordplay-grab.git; cd wordplay-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/wordplay-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, wordplay-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/wordplay-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/ArchiveTeam/wordplay-grab
    cd wordplay-grab; ./get-wget-lua.sh
